...
<nav class="navbar navbar-expand-lg navbar-light">
  ...
  <div class="col-md-5">
        <div class="product-text">
            ...
            <input type="hidden" id="max-stock" value="<?= htmlspecialchars($product['quantity']) ?>">
  ...
  fetch(`../../function/php/detail_product.php?id=${id}`)
    ...
    .then(data => {
      const currentMainProduct = {
        id: document.querySelector('#main-product-id').value,
        product_img: document.querySelector('#main-product-img').src.split('/').pop(),
        ...
        quantity: parseInt(document.querySelector('#main-product-stock').innerText.replace('Stock: ', ''))
      };

      // Update the main product details with the clicked product's details
      ...
      // UPDATE THE max-stock HIDDEN INPUT
      document.querySelector('#max-stock').value = data.quantity;
      
      // UPDATE THE URL IN THE BROWSER ADDRESS BAR
      const newUrl = `${window.location.pathname}?id=${data.id}&type=supplements`;
      window.history.pushState({path: newUrl}, '', newUrl);

      // IMPORTANT: Update the global variables for the new product
      unitCost = parseFloat(data.cost);
      maxStock = parseInt(data.quantity);
      
      // Update quantity input max attribute
      quantityInput.max = maxStock;
      
      // Reset quantity to 1 and update all displays
      syncQuantity(1);
      
      // Update hidden form fields for checkout with new product data
      document.querySelector('input[name="product-name"]').value = data.product_name;
      document.querySelector('input[name="cost"]').value = data.cost;
      document.querySelector('input[name="sub-total"]').value = data.cost;
      document.querySelector('input[name="total-amount"]').value = data.cost;
      
      // Update display values in checkout modal
      document.querySelector('#total-cost-1').textContent = `₱${parseFloat(data.cost).toFixed(2)}`;
      document.querySelector('#total-cost-2').textContent = `₱${parseFloat(data.cost).toFixed(2)}`;
      document.querySelector('#totalAmount').textContent = `₱${parseFloat(data.cost).toFixed(2)}`;

      // Move previous product to "You may also like" section (if it's not already there)
      ...
      prevProductItem.addEventListener('click', function() {
        fetchProductDetails(currentMainProduct.id);
      });

      // Remove the clicked product from "You may also like"
                       ...
                       <div class="form-check d-flex justify-content-between align-items-center mb-3 form-payment">
                              <div>
                                  <input type="radio" id="payment-cash" name="paymentMethod" 
                                        value="cash" class="form-check-input" required onchange="togglePaymentFields()">
                          ...
                          <div class="form-check d-flex justify-content-between align-items-center mb-3 form-payment">
                              <div>
                                  <input type="radio" id="payment-gcash" name="paymentMethod" 
                                        value="gcash" class="form-check-input" required onchange="togglePaymentFields()">
                        ...
                        <div id="gcash-details" style="display: none;">
                            <img src="../../../../assets/img/gcash.jpg" alt="Gcash" class="img-fluid mb-3 gcash">
                            ...
                            <input type="text" name="reference" value="" class="form-control mt-1" 
...
let unitCost = <?php echo $product['cost']; ?>;
let maxStock = <?php echo $product['quantity']; ?>; // Get max stock from PHP

const quantityInput = document.getElementById("quantity");
const incrementBtn = document.getElementById("increment-btn");
const decrementBtn = document.getElementById("decrement-btn");
const shippingFeeElement = document.getElementById("shippingFee");
const shippingFee = parseFloat(shippingFeeElement.textContent);

// Function to update button states
function updateButtonStates() {
    const currentValue = parseInt(quantityInput.value);
    
    // Disable decrement button if value is 1
    decrementBtn.disabled = currentValue <= 1;
    
    // Disable increment button if value equals max stock
    incrementBtn.disabled = currentValue >= maxStock;
    ...
    if (incrementBtn.disabled) {
        incrementBtn.classList.add('disabled-btn');
    } else {
        incrementBtn.classList.remove('disabled-btn');
    }
}
...
function updateTotalCost(quantity, totalCostElement) {
    ...
    totalCostElement.textContent = `₱${totalCost} PHP`;
...
function showCartModal() {
  ...
  // Use the updated maxStock variable, not the PHP one
  if (quantity > maxStock) {
    alert(`Only ${maxStock} items available in stock.`);
    document.querySelector('#quantity').value = maxStock;
    syncQuantity(maxStock);
    return;
  }
...
function syncQuantity(value) {
    ...
    const subtotal = updateTotalCost(value, totalCostDisplay1);
    updateTotalCost(value, totalCostDisplay2);
...
// Add CSS for disabled buttons (add this to your CSS file or style tag)
...
document.head.appendChild(style);
    ...
    document.getElementById('product_img').value = productImage;
      ...
      <script>
// Add GCash validation function
function validateGCashFields() {
    var gcashChecked = document.getElementById("payment-gcash").checked;
    
    if (gcashChecked) {
        var gcashImage = document.getElementById("gcash-image");
        var gcashReference = document.querySelector('input[name="reference"]');
...
// Update the payment method toggle function
...
const gcashReference = document.querySelector('input[name="reference"]');
...
document.querySelector('form[action="../../function/php/checkout_process.php"]').addEventListener('submit', function(event) {
