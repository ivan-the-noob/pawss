...
if (!empty($searchKeyword)) {
    $searchQuery .= " AND (
        owner_name LIKE '%$searchKeyword%' OR 
...
$sqlCount = "SELECT COUNT(*) AS total FROM appointment WHERE $searchQuery";
...
$sql = "SELECT * FROM appointment WHERE $searchQuery LIMIT $offset, $itemsPerPage";
...
if (!$result) {
    ...
    if ($result->num_rows > 0) {
        $count = $offset + 1; // Adjust for the offset
...
$paginationLinks .= "<li class='page-item'><a class='page-link' href='?search=$searchKeyword&page=$prevPage'>&lt;</a></li>";

// Page numbers
for ($i = 1; $i <= $totalPages; $i++) {
    $activeClass = ($i == $page) ? " active" : "";
    $paginationLinks .= "<li class='page-item$activeClass'><a class='page-link' href='?search=$searchKeyword&page=$i'>$i</a></li>";
}

// "Next" button
$nextPage = $page < $totalPages ? $page + 1 : $totalPages;
$paginationLinks .= "<li class='page-item'><a class='page-link' href='?search=$searchKeyword&page=$nextPage'>&gt;</a></li>";

// Output pagination links
echo "<script>
    document.getElementById('pageNumbers').innerHTML = `$paginationLinks`;
</script>";
?>
